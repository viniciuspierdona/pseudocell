...
public class EditRule extends JFrame {
    ...
    private int ruleType, bracketNumber, ruleIndex;
     ...
     * @param selectedRule
     */
    public EditRule(Config configurations, Node selectedNode, List<Node> nodeList, Rule selectedRule) {
    ...
    public void shown() {
        Rule r;
        for (int i=0; i<selectedNode.getRules().size(); i++) {
            r = selectedNode.getRules().get(i);
            if(r.equals(selectedRule)) {
                ruleIndex = i;
            }
        }
    ...
    private void setComponents() {
        ...
        setTitle("Pseudocell - Edit the rules that compose your logical network");
        ...
        jLabel_Subtitle2.setText("Edit rule");
        ...
        jTree_Rule.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent evt) {
                jTree_Rule_MouseClicked(evt);
            }
        });
    }
    ...
    /**
     * Loads information from the selected rule fragment into the edit fields.
     * 
     * @param evt 
     */
    private void jTree_Rule_MouseClicked(MouseEvent evt) {
        // Ensures the item will only be accessed on the second click
        if (evt.getClickCount() == 2) {
            DefaultMutableTreeNode treeNode = (DefaultMutableTreeNode) jTree_Rule.getLastSelectedPathComponent();
            ...
            if (treeNode != null && treeNode.isLeaf()) {
                ...
                // Iterates over all the nodes in order to retrieve the information of the node involved in the operational relationship and loads them into the edit fields
                for (Node node : nodeList) {
                    if (node.getName().equals(ruleFragmentVariables.get(0))) {
                        jComboBox_RO_Node.setSelectedItem(node);
                ...
                // Loads the other information from the operational relation to its fields
                jComboBox_ComparisionVariable.setSelectedItem(ruleFragmentVariables.get(1));
                jComboBox_RelationalOperation.setSelectedItem(ruleFragmentVariables.get(2));
    ...
    /**
     * Identifies and separates the elements of the selected rule fragment
     * 
     * @param ruleFragment
     * @return List
     */
    ...
    private void jButton_CreateRule_ActionPerformed() {
        ...
        selectedNode.getRules().set(ruleIndex, rule);
    ...
    /**
     * Populates the fields of the editing panel according to the information of the selected rule.
     * 
     */
    private void setRuleFields() {
        // Populates the JTree with the conditionals of the selected rule
        jTree_Rule.populateTree(selectedRule);

        // Enables and populates fields according to the type of rule [general, metabolic, modification]
        if (selectedRule instanceof MetabolicRule) {
            ruleType = RULETYPE_METABOLIC;
            for (Node n : nodeList) {
                if (n.getName().equals(selectedRule.getInputNode())) {
                    jComboBox_InputNode.setSelectedItem(n);
                } else if (n.getName().equals(selectedRule.getOutputNode())) {
                    jComboBox_OutputNode.setSelectedItem(n);
                }
            }
            //jSpinner_ConversionValue.setValue(selectedRule.getNodeStatus());
        } else if (selectedRule instanceof ModificationRule) {
            ruleType = RULETYPE_MODIFICATION;
            for (NodeModification mod : selectedNode.getNodeModifications()) {
                if (mod.getName().equals(selectedRule.getModification())) {
                    jComboBox_Modification.setSelectedItem(mod);
                }
            }
            ...
            ruleType = RULETYPE_DEFAULT;
            jTextField_RuleStatus.setText(String.valueOf(selectedRule.getNodeStatus()));
        }
        setRuleTypeFields();
    }
    
    /**
     * Enables the fields referring to the selected rule type and disables the others.
     * 
     */
