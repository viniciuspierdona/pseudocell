 ...
 */
public class OperationParticle {

    private LogicalOperation operator = null;
    private OperationParticle leftNode = null;
    private OperationParticle rightNode = null;
    private Double value = null;

    /**
     * Create a OperationParticle using the given expression.
     *
     * @param s the expression
     */
    public OperationParticle(LogicalExpression s) {
        this(s.getExpression(), s);
    }

    private OperationParticle(String s, LogicalExpression exp) {
        ...
        for (int i = 0; i < sLength; i++) {
            if (s.charAt(i) == '(') {
                ...
                if (inBrackets == 0) {
                    LogicalOperation o = getOperator(s, i);
                    if (o != null) {
                        if (operator == null
        ...
        if (operator != null) {
            ...
            if (startOperator == 0 && operator.getType() == Operands.SINGLE) {
                if (checkBrackets(s.substring(operator.getOperator().length()))) {
                    leftNode = new OperationParticle(s.substring(operator.getOperator()
                            .length()), exp);
                    return;
                }
                ...
                leftNode = new OperationParticle(s.substring(0, startOperator), exp);
                rightNode = new OperationParticle(s.substring(startOperator
     ...
     *
     * @param s the expression part
     * @param start the start index
     * @return the operator
     */
    public LogicalOperation getOperator(String s, int start) {
        LogicalOperation[] operators = LogicalOperation.values();
    ...
    /**
     * @param s the part of expression
     ...
     *
     * @param s the operation to check
     ...
     */
    public OperationParticle getLeft() {
        return leftNode;
    }
     ...
     */
    public OperationParticle getRight() {
        return rightNode;
    }
